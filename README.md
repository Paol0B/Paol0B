# 👋 Hi, I'm Paol0B

Senior Software Developer & Open Source Contributor  
Building **secure**, **high-performance**, and **modern software** across systems and application layers with a passion for clean design, robust tooling, and cybersecurity.

---

## About Me

- **Senior Software Developer** with extensive experience in multiple languages and paradigms  
- Focused on **security-first development** and performant code  
- Strong contributor to open source projects, including  a cross-platform Rust reimplementation of GNU core utilities

---

## Expertise

### 🔹 Primary Languages
- **.NET / C#** — backend systems, libraries, frameworks
- **Rust** — safe systems programming, high-performance tooling
- **Zig** — modern low-level systems
- **Kotlin & Java** — application and cross-platform development
- **C & C++** — performance-critical and systems software

### 🔹 Cybersecurity
- Secure coding practices
- Defensive tool design
- Threat-aware implementation strategies

---

## Open Source Work

- Active contributor to [uutils/coreutils](https://github.com/uutils/coreutils) — a cross-platform Rust rewrite of the GNU coreutils.
- Developed and maintained libraries and tools that demonstrate real-world secure and high-quality engineering practices

---

## What I Build

- **Secure libraries** & cryptographic tools  
- **Command-line tools & utilities**  
- **Cross-platform systems software**  
- **Modern backend services**

